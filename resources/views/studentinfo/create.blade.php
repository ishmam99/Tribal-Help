    ...
    <div class="container-fluid">
            ...
            <div class="animated fadeIn delay-1s">
                <div class="page-title-wrapper container-fluid" id="showitem">
                    <div class="page-title-heading d-flex">
                        ...
                        <div id="showitem">
                            <div><h4>নতুন শিক্ষার্থী এন্ট্রি করুন</h4></div>
        ...
        <div class=" mt-3">
            <div class="animated fadeIn">
                {{-- form --}}
                <form action="{{url()->to('students')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div id="msg">
                        <div class="card">
                                ...
                                <div class="row">
                                        ...
                                        <div class="row">
                                            <div class="col-md-4 col-sm-6 col-12">
                                                <div class="form-group">
                                                    <label for="application_date" class=" form-control-label" >* তারিখ</label>
                                    ...
                                    @if($item_prop->inputType>=0&&$item_prop->inputType<=3)
                                     <div class="col-md-6 col-12">
                                        <div class="form-group">
                                            ...
                                            <input type="{{$input_type_array[$item_prop->inputType]}}" id="{{$item_prop->sql}}" placeholder="{{$item_prop->name}}" class="form-control" name="{{$item_prop->sql}}" @if($item_prop->isRequired) {{"required"}} @endif >
                                            @error('{{$item_prop->sql}}')
                                            <div class="invalid-feedback">
                                                {{$message}}
                                            </div>
                                    ...
                                    @elseif($item_prop->inputType==4)
                                     <div class="col-md-12 col-12">
                                        <div class="form-group">
                                            <label for="{{$item_prop->sql}}" class=" form-control-label" > @if($item_prop->isRequired) {{"*"}} @endif {{$item_prop->name}}</label>
                                            <textarea  id="{{$item_prop->sql}}" placeholder="{{$item_prop->name}}" class="form-control" name="{{$item_prop->sql}}"  cols="5" rows="1" @if($item_prop->isRequired) {{"required"}} @endif ></textarea>
                                            @error('{{$item_prop->sql}}')
                                            <div class="invalid-feedback">
                                                {{$message}}
                                            </div>
                                    ...
                                    @if( $item_prop->sql==="upazila_name" && $schooladmin )
                                    <input type="hidden"  value="{{$schoolinfo->upazila_name}}" name="upazila_name" id="upazila_name" />
                                    @elseif( $item_prop->sql==="school_id" && $schooladmin )
                                    <input type="hidden"  value="{{$schoolinfo->id}}" name="school_id" id="school_id" />
                                    @else
                                     <div class="col-md-6 col-12">
                                        <div class="form-group">
                                            <label for="{{$item_prop->sql}}" class=" form-control-label" > @if($item_prop->isRequired) {{"*"}} @endif {{$item_prop->name}}</label>
                                            <select  id="{{$item_prop->sql}}"  style="color:#312e2e" placeholder="{{$item_prop->name}}" class="form-control" name="{{$item_prop->sql}}"  @if($item_prop->isRequired) {{"required"}} @endif >
                                                <?php echo $optionArray[$item_prop->sql]; ?>
                                            </select>
                                            @error('{{$item_prop->sql}}')
                                            <div class="invalid-feedback">
                                                {{$message}}
